...
import 'package:flowkit/helpers/widgets/my_page_indicator.dart';
...
class MyPageDragger extends StatefulWidget {
  ...
  MyPageDragger({super.key, this.canDragLeftToRight, this.canDragRightToLeft, this.slideUpdateStream});
...
class _MyPageDraggerState extends State<MyPageDragger> {
  ...
  Widget build(BuildContext context) {
    return GestureDetector(onHorizontalDragStart: onDragStart, onHorizontalDragUpdate: onDragUpdate, onHorizontalDragEnd: onDragEnd);
  }
}
  ...
  AnimatedPageDragger({
    ...
    completionAnimationController = AnimationController(duration: duration, vsync: vsync)
      ..addListener(() {
        ...
        slideUpdateStream!.add(SlideUpdate(UpdateType.animating, slideDirection, slidePercent));
      })
      ..addStatusListener((AnimationStatus status) {
        if (status == AnimationStatus.completed) {
          slideUpdateStream!.add(SlideUpdate(UpdateType.doneAnimating, slideDirection, endSlidePercent));
        }
      });
  }
